// Sistema de Gerenciamento de Progresso
        class ProgressManager {
            ...
            updateAllButtons() {
                document.querySelectorAll('.complete-day-btn').forEach(btn => {
                    ...
                    if (dayId && this.isDayComplete(dayId)) {
                        ...
                        btn.innerHTML = '<i class="fas fa-check-circle"></i> <span class="btn-text">Concluído</span>';
                    } else if (dayId) {
                        btn.classList.remove('completed');
                        btn.innerHTML = '<i class="far fa-check-circle"></i> <span class="btn-text">Marcar como concluído</span>';
                    }
                });
            ...
            importProgress(base64String) {
                try {
                    if (!base64String.trim()) {
                        return { success: false, message: 'Código vazio. Por favor, cole um código válido.' };
                    }
                    ...
                    if (data.progress && data.version === '1.0') {
                        ...
                        return { success: false, message: 'Código inválido ou versão incompatível.' };
                    }
                } catch (error) {
            ...
            markCurrentWeekComplete() {
                ...
                if (marked > 0) {
                    this.saveProgress();
                    alert(`Marcados ${marked} períodos da semana ${weekNumber} como concluídos!`);
                } else {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function setupWeekNavigation() {
                document.addEventListener('click', function (e) {
                    if (e.target.classList.contains('nav-week-btn')) {
                        const weekBtns = e.target.closest('.week-navigation').querySelectorAll('.nav-week-btn');
                        const weekContents = e.target.closest('.schedule-content').querySelectorAll('.week-content');
            ...
            document.getElementById('export-progress-btn').addEventListener('click', function () {
                const code = progressManager.exportProgress();
                if (!code) {
                    syncStatus.className = 'sync-status error';
                    syncStatus.innerHTML = '<strong>Erro ao gerar código. Tente novamente.</strong>';
            ...
            document.getElementById('import-progress-btn').addEventListener('click', function () {
                ...
                setTimeout(() => {
                    document.getElementById('performImportBtn').addEventListener('click', function () {
            ...
            document.getElementById('sync-cloud-btn').addEventListener('click', function () {
            ...
            document.getElementById('modal-reset-btn').addEventListener('click', function () {
                if (confirm('Tem certeza que deseja limpar todo o seu progresso? Esta ação não pode ser desfeita.')) {
                    ...
                    syncStatus.className = 'sync-status success';
                    syncStatus.innerHTML = '<strong>✓ Progresso limpo com sucesso!</strong>';
                    syncStatus.style.display = 'block';
                    setTimeout(() => {
            ...
            document.getElementById('reset-progress').addEventListener('click', function () {
                if (confirm('Tem certeza que deseja reiniciar todo o progresso da semana atual?')) {
            ...
            const examDateFormatted = examDate.toLocaleDateString('pt-BR', options);

            const dateInfo = document.createElement('div');
            dateInfo.className = 'date-info';
            dateInfo.innerHTML = `<strong><i class="far fa-calendar-check"></i> Data estimada da prova:</strong> ${examDateFormatted} (13 semanas a partir de hoje)`;

            document.querySelector('.header-info').appendChild(dateInfo);
        });
